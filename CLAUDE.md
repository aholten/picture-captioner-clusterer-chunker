...
## Context
`cluster_report.py` currently assigns labels via CLIP vocabulary matching — picking the best phrase
from a fixed list. This is fundamentally limited. The user wants genuine per-photo image captions
generated by a vision-language model, with the captioning backend abstracted so the job can be
run on different hardware (current RTX 3070 vs future better GPU) or routed to cloud API providers.
The job must be resumable since 44k photos takes hours.
...
All new work lives in `v2/`. Existing top-level scripts are POC reference only — untouched.

```
icloud-to-immich/
├── PLAN.md                          ← this plan, saved as a top-level doc
├── cluster.py                       ← POC (do not edit)
├── cluster_report.py                ← POC (do not edit)
├── label_cluster.py                 ← POC (do not edit)
├── dedup.py                         ← POC (do not edit)
├── ...
└── v2/
...
### Dependency management (`v2/pyproject.toml`)

```toml
[project]
name = "icloud-to-immich-v2"
version = "0.1.0"
...
Usage:
```bash
uv run --extra local  v2/caption.py --backend local ...
uv run --extra api    v2/caption.py --backend openai ...
uv run --extra report v2/cluster_report.py
uv run --extra dev    pytest v2/tests/
```
...
```
uv run --extra local v2/caption.py run \
    --backend local \
...
uv run --extra api v2/caption.py run \
...
uv run --extra local v2/caption.py run --backend local --force
```
...
uv run v2/caption.py run --backend mock --dry-run --photos-dir ~/icloud_photos
...
```bash
# run_loop.sh — provided in v2/ for the full production run
#!/usr/bin/env bash
set -e
BACKEND="${1:-local}"
...
`run_loop.sh` is added to `v2/` as a convenience wrapper for the production run.
...
**Config via `pydantic-settings` (`v2/config.py`):**
```python
from pydantic_settings import BaseSettings, SettingsConfigDict

class Settings(BaseSettings):
    model_config = SettingsConfigDict(env_file="config.env", env_file_encoding="utf-8")
    DOWNLOAD_DIR: Path              # required — no default
    IMMICH_URL: str = ""
    IMMICH_API_KEY: str = ""
    OPENAI_API_KEY: str = ""
...
**Pydantic models for JSONL records (`v2/journal.py`):**
```python
...
**Portability:** `rel_path` is relative to `--photos-dir` (defaults to `DOWNLOAD_DIR` from
...
### Image preprocessing layer (`v2/image_loader.py`)
...
API keys read from `config.env` (existing project pattern):
- `GEMINI_API_KEY`
- `OPENAI_API_KEY`
- `ANTHROPIC_API_KEY`
- `XAI_API_KEY`
...
## Changes to `v2/cluster_report.py`
...
## Files to create (nothing in the top-level project is edited)

| File | Description |
|---|---|
| `icloud-to-immich/PLAN.md` | This plan, saved as a top-level project document |
| `icloud-to-immich/v2/README.md` | Setup guide, usage, full pipeline walkthrough |
| `icloud-to-immich/v2/pyproject.toml` | Project metadata + optional dep groups |
| `icloud-to-immich/v2/config.py` | `pydantic-settings` Settings — validates config.env |
| `icloud-to-immich/v2/journal.py` | **Append-only journal** — sole owner of captions.jsonl I/O |
| `icloud-to-immich/v2/caption.py` | CLI orchestrator only — typer subcommands, loop logic |
| `icloud-to-immich/v2/run_loop.sh` | Shell loop for production run — restarts every N images to clear GPU/RAM |
| `icloud-to-immich/v2/cluster_report.py` | Cluster report + CSV, reads captions.jsonl via `journal.py` |
| `icloud-to-immich/v2/backends/__init__.py` | `BACKENDS` registry dict + `load_backend()` factory |
| `icloud-to-immich/v2/image_loader.py` | `load_image()` — HEIC, EXIF strip, RGB, resize, orientation |
| `icloud-to-immich/v2/backends/base.py` | `CaptionBackend` ABC, `CorruptImageError` |
| `icloud-to-immich/v2/backends/mock.py` | `MockBackend` — no deps, instant fake captions |
| `icloud-to-immich/v2/backends/local_hf.py` | `LocalHFBackend` — torch, transformers, bitsandbytes |
| `icloud-to-immich/v2/backends/openai_api.py` | `OpenAIBackend` + `XAIBackend` |
| `icloud-to-immich/v2/backends/anthropic_api.py` | `AnthropicBackend` |
| `icloud-to-immich/v2/backends/gemini_api.py` | `GeminiBackend` |
| `icloud-to-immich/v2/docs/api_providers.md` | API provider pricing + privacy policy reference |
| `icloud-to-immich/v2/docs/self_hosted_models.md` | Self-hosted model comparison + hardware requirements |
| `icloud-to-immich/v2/docs/adding_a_backend.md` | 10-line template + step-by-step guide for new backends |
| `icloud-to-immich/v2/docs/caption_samples.md` | Sample captions from each backend on the same 5 test photos |
| `icloud-to-immich/v2/tests/conftest.py` | Shared test fixtures |
| `icloud-to-immich/v2/tests/test_caption.py` | Unit tests using MockBackend |
| `icloud-to-immich/v2/tests/test_journal.py` | Unit tests for journal: atomicity, resume, corrupt-line handling |

**Reference only (do not edit):**
- `icloud-to-immich/cluster_report.py` — pattern reference for UMAP/HDBSCAN/CSV logic
- `icloud-to-immich/cluster.py` — pattern reference for embedding cache + config.env parsing

---

## Append-only journal (`v2/journal.py`)
...
```bash
uv run --extra dev pytest v2/tests/ -v
```
...
```bash
uv run --extra api v2/caption.py --backend mock --limit 20 --photos-dir ~/icloud_photos
```
**Check:**
...
uv run v2/caption.py --backend mock --limit 30 --photos-dir ~/icloud_photos
# (Ctrl+C)
...
```bash
# With mock captions.jsonl (30 photos), run v2 report
uv run --extra report v2/cluster_report.py
```
**Check:**
...
uv run --extra local v2/caption.py run --backend local --model Qwen/Qwen2.5-VL-3B-Instruct --limit 10 --photos-dir ~/icloud_photos
```
**Check:**
...
uv run --extra api v2/caption.py --backend openai --model gpt-4o-mini --limit 5 --photos-dir ~/icloud_photos
```
...
bash v2/run_loop.sh local Qwen/Qwen2.5-VL-3B-Instruct
